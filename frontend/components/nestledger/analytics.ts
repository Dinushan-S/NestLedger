...
export function priorPeriod(period: AnalysisPeriod): {
	start: Date;
	end: Date;
} {
...
function buildGroups(
	...
	if (borrowNet > 0)
		map.set(BORROW_CATEGORY, { spent: borrowNet, count: borrowCount });
	return map;
}

function totalOf(
	groups: Map<string, { spent: number; count: number }>,
): number {
	let t = 0;
...
export interface AnalyzeInput {
	...
	priorRange?: { start: Date; end: Date } | undefined;
	planTotal?: number | null;
...
function buildRecommendations(args: {
	...
	if (planTotal !== null && totalSpent > planTotal) {
		...
		const driver = top
			? ` The biggest category was ${top.category} at ${fmt(top.spent)}.`
			: "";
