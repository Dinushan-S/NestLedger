...
export function buildCurrentMonthBillStatsMap(
	...
	billTrackers.forEach((tracker) => {
		const trackerBills = recurringBills.filter(
			(bill) => bill.tracker_id === tracker.id,
		);
		const trackerPayments = billPayments.filter(
			(payment) => payment.tracker_id === tracker.id,
		);
		...
		const pending = pendingBills.reduce(
			(sum, bill) => sum + bill.default_amount,
			0,
		);
...
export function buildBorrowBalances(
	expenses: Expense[],
): Map<string, { borrowed: number; repaid: number }> {
	const balances = new Map<string, { borrowed: number; repaid: number }>();
	...
	expenses.forEach((expense) => {
		...
		const current = balances.get(userId) ?? { borrowed: 0, repaid: 0 };

		if (expense.price > 0) {
			current.borrowed += expense.price;
		} else {
			current.repaid += Math.abs(expense.price);
		}
