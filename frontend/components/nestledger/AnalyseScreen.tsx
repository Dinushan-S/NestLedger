...
import { buildBorrowBalances } from "./selectors";
...
export default function AnalyseScreen({
	...
	const nameOf = useMemo(
		() => (id: string | null | undefined) =>
			(id && members.find((m) => m.user_id === id)?.user_profile?.name) ||
			"Family",
		[members],
	);
	...
	useEffect(() => {
		...
		if (cursorBounds) {
			...
			const clamped = {
				year: Math.max(
					minCursor.year,
					Math.min(maxCursor.year, initialCursor.year),
				),
				month:
	...
	const whoOwes = useMemo(() => {
		const balances = buildBorrowBalances(cycleExpenses);
		return [...balances.entries()]
			.map(([uid, balance]) => ({
				uid,
				net: Math.round((balance.borrowed - balance.repaid) * 100) / 100,
			}))
			.filter((r) => r.net > 0)
	...
	const detailRows = useMemo(() => {
		...
		const rows = cycleExpenses.filter((x) =>
			detailCategory === BORROW_CATEGORY
				? x.is_borrow
	...
	const remaining =
		planTotal != null ? Math.round((planTotal - totalSpent) * 100) / 100 : null;
	...
	const stepCycle = (delta: number) => {
		setCursor((cur) => {
			...
			const { minCursor, maxCursor } = cursorBounds;
			if (
				next.year < minCursor.year ||
				(next.year === minCursor.year && next.month < minCursor.month)
			) {
				return minCursor;
			}
			if (
				next.year > maxCursor.year ||
				(next.year === maxCursor.year && next.month > maxCursor.month)
			) {
				return maxCursor;
			}
			return next;
	...
	const visibleCategories = analysis
		? analysis.categories.filter((c) => c.spent > 0)
		: [];
	const cycleLabel = cycle
		? `${fmtDate(cycle.start)} – ${fmtDate(cycle.nextAnchor)}`
		: "";
			...
			<View style={styles.rowMain}>
				...
				<View style={styles.shareTrack}>
					<View
						style={[
							styles.shareFill,
							{ width: `${Math.max(sharePct, 2)}%`, backgroundColor: color },
						]}
					/>
				</View>
				<View style={styles.rowBottom}>
					...
					{td && (
						<View style={styles.rowMeta}>
							<Text style={[styles.rowTrend, { color: td.color }]}>
								{td.label}
							</Text>
