...
export function SavingsTracker({
	...
	const deposits = useMemo(
		() => filteredSavings.filter((e) => e.amount > 0),
		[filteredSavings],
	);
			...
			<View style={s.headerRow}>
				<View style={s.headerContent}>
					...
					<Text style={s.bodyMuted}>
						{(stats?.balance ?? totalSavings) >= 0
							? "Total saved"
							: "Net borrowed"}
						:{" "}
						{formatCurrency(
							Math.abs(stats?.balance ?? totalSavings),
							currencyCode,
						)}
					</Text>
				</View>
			...
			<View style={s.statRow}>
				<View style={s.statPill}>
					...
					<Text
						style={[
							s.statPillValue,
							(stats?.balance ?? totalSavings) < 0 && { color: theme.danger },
						]}
					>
				...
				<View style={s.statPill}>
					...
					<Text
						style={[
							s.statPillValue,
							(stats?.net ?? thisMonthNet) < 0
								? { color: theme.danger }
								: { color: theme.success },
						]}
					>
						{(stats?.net ?? thisMonthNet) >= 0 ? "+" : ""}
						{formatCurrency(stats?.net ?? thisMonthNet, currencyCode)}
					</Text>
				...
				<View style={s.statPill}>
					...
					<Text style={s.statPillValue}>
						{formatCurrency(
							stats?.deposits ?? deposits.reduce((s, e) => s + e.amount, 0),
							currencyCode,
						)}
					</Text>
					<Text style={s.statPillSub}>
						{stats ? "" : `${deposits.length} entries`}
					</Text>
				</View>
				<View style={s.statPill}>
					...
					<Text style={s.statPillSub}>
						{stats ? "" : `${withdrawals.length} entries`}
					</Text>
				</View>
			</View>
							...
							{entry.linked_plan_id ? (
								<View style={s.planBadge}>
									<Text style={s.planBadgeText}>
										{plans.find((p) => p.id === entry.linked_plan_id)?.name ??
											"Plan"}
									</Text>
								</View>
							) : null}
			...
			{filteredSavings.length > 10 ? (
				...
				>
					<Text style={s.linkText}>
						View all {filteredSavings.length} entries
					</Text>
				</Pressable>
			) : null}
						...
						<>
							<BentoCard tone="highlight">
								<View style={s.detailRow}>
									<Ionicons
										color={
											selectedEntry.amount >= 0 ? theme.success : theme.danger
										}
										name={
											selectedEntry.amount >= 0 ? "add-circle" : "remove-circle"
										}
										size={32}
									/>
									<View style={{ flex: 1 }}>
										<Text style={s.detailAmount}>
											...
											{formatCurrency(selectedEntry.amount, currencyCode)}
										</Text>
										<Text style={s.bodyMuted}>
								...
								<View style={s.detailMeta}>
									<Text style={s.bodyMuted}>
										Date: {formatShortDate(selectedEntry.date)}
									</Text>
									{selectedEntry.note ? (
									...
									{selectedEntry.linked_plan_id ? (
										<Text style={s.bodyMuted}>
											Plan:{" "}
											{plans.find((p) => p.id === selectedEntry.linked_plan_id)
												?.name ?? "Unknown"}
										</Text>
									) : null}
								</View>
