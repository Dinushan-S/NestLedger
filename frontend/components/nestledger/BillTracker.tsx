...
export function BillTracker({
	...
	const billPaymentMap = useMemo(() => {
		const map: Record<
			string,
			{ paid: number; pending: number; payments: BillPayment[] }
		> = {};
	...
	const handleCategoryChange = (cat: string) => {
		setBillForm((f) => ({
			...
			name:
				cat === "Other"
					? ""
					: `${cat} - ${monthNames[new Date().getMonth()]} ${new Date().getFullYear()}`,
		}));
	};
	...
	const handleSubmitBill = () => {
		const finalName =
			billForm.category === "Other" && !billForm.name.trim()
				? "Other"
				: billForm.name.trim();
		if (!finalName || !billForm.dueDay.trim() || !billForm.amount.trim())
			return;
		onAddBill({
			...
			default_units:
				needsUnits && billForm.units.trim() ? Number(billForm.units) : null,
	...
	return (
		<View>
			<View style={s.headerRow}>
				<View style={s.headerContent}>
					...
					<Text style={s.bodyMuted}>
						This month: {formatCurrency(totalPaidAmount, currencyCode ?? "USD")}{" "}
						paid for {paidThisMonth.length} bills
					</Text>
				</View>
				<ModernButton
					onPress={handleOpenComposer}
					secondary
					testID="bill-open-composer"
					text="New Bill"
				/>
			</View>
			...
			<View style={s.statRow}>
				<StatPill
					label="Paid"
					sub={`${stats?.paidCount ?? paidThisMonth.length} bills`}
					value={formatCurrency(totalPaidAmount, currencyCode)}
				/>
				<StatPill
					label="Pending"
					sub={`${stats?.pendingCount ?? pendingThisMonth.length} bills`}
					value={formatCurrency(totalPendingAmount, currencyCode)}
					valueColor={theme.warning}
				/>
				<StatPill
					label="Total"
					sub={`${stats?.totalCount ?? thisMonthPayments.length} entries`}
			...
			<View style={s.billList}>
				{filteredBills.length === 0 ? (
					...
					filteredBills.map((bill) => {
						const stats = billPaymentMap[bill.id] ?? {
							paid: 0,
							pending: 0,
							payments: [],
						};
							...
							>
								...
								<View style={s.billInfo}>
									...
									<Text style={s.billMeta}>
										...
										{bill.default_amount > 0
											? ` · ${formatCurrency(bill.default_amount, currencyCode)}`
											: ""}
										{bill.notify_days_before > 0
											? ` · ${bill.notify_days_before}d before`
											: ""}
									</Text>
								</View>
								...
								<Pressable onPress={(e) => e.stopPropagation()}>
									...
									<View style={s.inputGroup}>
										...
										<TextInput
											keyboardType="numeric"
											onChangeText={(v) =>
												setBillForm((f) => ({ ...f, notifyDaysBefore: v }))
											}
											placeholder="e.g. 3"
											placeholderTextColor={theme.textMuted}
											style={s.input}
									...
									<View style={s.toggleRow}>
										...
										<Pressable
											...
											style={[
												s.toggleTrack,
												billForm.isRecurring && s.toggleTrackActive,
											]}
										...
										>
											<View
												style={[
													s.toggleThumb,
													billForm.isRecurring && s.toggleThumbActive,
												]}
											/>
										</Pressable>
									</View>
									...
									<View style={s.toggleRow}>
										...
										<Pressable
											hitSlop={10}
											onPress={() =>
												setBillForm((f) => ({ ...f, isActive: !f.isActive }))
											}
											style={[
												s.toggleTrack,
												billForm.isActive && s.toggleTrackActive,
											]}
										...
										>
											<View
												style={[
													s.toggleThumb,
													billForm.isActive && s.toggleThumbActive,
												]}
											/>
										</Pressable>
									</View>
											...
											<View style={s.segmentRow}>
												{plans.map((plan) => (
													<CategoryChip
														...
														label={plan.name}
														onPress={() =>
															setPaymentForm((f) => ({ ...f, planId: plan.id }))
														}
										...
										<View style={s.segmentRow}>
											...
											{members.map((m) => (
												<CategoryChip
													...
													onPress={() =>
														setPaymentForm((f) => ({ ...f, paidBy: m.user_id }))
													}
							...
							<BentoCard>
								<View style={s.detailHeader}>
									<View style={s.billIconWrap}>
										<Ionicons
											color={theme.primary}
											name={getCategoryIcon(detailBill.category)}
											size={28}
										/>
									</View>
									<View style={{ flex: 1 }}>
										...
										<Text style={s.bodyMuted}>
											{detailBill.category} &middot; Due day{" "}
											{detailBill.due_day}
										</Text>
										<Text style={s.bodyMuted}>
											{detailBill.is_recurring ? "Recurring" : "One-time"}{" "}
											&middot; {detailBill.is_active ? "Active" : "Inactive"}
										</Text>
										<Text style={s.bodyMuted}>
