...
const billPaymentColumns =
	"id, profile_id, tracker_id, bill_id, plan_id, amount, units, status, date, month, year, added_by, created_at";
...
/**
 * Shared CRUD shape for simple single-table resources: every create stamps
 * created_at, and updates/deletes target the row by id. Resources with custom
 * payloads or post-processing (expenses, payments, notifications) keep their
 * own methods and are intentionally not routed through here.
 */
function tableCrud<T extends { created_at: string; id: string }>(
	table: string,
	options?: { columns?: string },
) {
	return {
		async create(input: Omit<T, "created_at" | "id">) {
...
export const profileApi = {
	...
	async fetchMembers(profileId: string) {
		...
		const userIds = [
			...new Set((memberships ?? []).map((item) => item.user_id)),
		];
		...
		const profileMap = new Map(
			(userProfiles ?? []).map((item) => [item.user_id, item]),
		);
	...
	async updateHousehold(
		...
		values: Partial<
			Pick<
				HouseholdProfile,
				| "bill_tracker_enabled"
				| "emoji_avatar"
				| "name"
				| "savings_tracker_enabled"
				| "space_type"
			>
		>,
	) {
	...
	async deleteHousehold(session: Session, profileId: string) {
		return callBackend("/spaces/delete", session.access_token, {
...
export const budgetApi = {
	createPlan: planCrud.create,
	updatePlan: planCrud.update,
	async deletePlan(planId: string) {
		...
		if (!data || data.length === 0) {
			throw new Error(
				"Unable to delete budget plan. You may not have permission.",
			);
		}
	},
...
export const shoppingApi = {
	async addItem(
		input: Omit<
			ShoppingItem,
			"bought_at" | "bought_by" | "created_at" | "id" | "is_bought"
		>,
	) {
		const payload = {
			...input,
...
export const inviteApi = {
	async acceptInvite(session: Session, inviteToken: string) {
		return callBackend<{ profile_id: string }>(
			"/invitations/accept",
	...
	async sendInvite(session: Session, payload: InvitePayload) {
		...
		}>("/invitations/send", session.access_token, payload);
	},
};

export const pushApi = {
	async fanOut(session: Session, payload: Record<string, unknown>) {
		return callBackend<{ sent: number }>(
			"/push/fanout",
			session.access_token,
			payload,
		);
...
export const billApi = {
	...
	createTracker: billTrackerCrud.create,
	updateTracker: billTrackerCrud.update,
	deleteTracker: billTrackerCrud.remove,
...
export const savingsApi = {
	...
	createTracker: savingsTrackerCrud.create,
	updateTracker: savingsTrackerCrud.update,
	deleteTracker: savingsTrackerCrud.remove,
	...
	addEntry: savingsEntryCrud.create,
	deleteEntry: savingsEntryCrud.remove,
};
